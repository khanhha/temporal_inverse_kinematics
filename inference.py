...
from pose_trainer import IKPoseTrainer
...
def run_inference(model, seq_3d_kps):
    ...
    h_w_size = 0# model.hparams.win_size // 2
...
def run_test():
    ...
    seq_kps_3d_path = Path(
        '/media/F/moveai/data/MOX_projects/MOX-017-RED/run_2020-08-18-15-51-16-main-dancer_monocular/poses_3d/cam01_track00.npz')
    ...
    ckpt_path = Path('/media/F/datasets/amass/ik_model/model_ckps/checkpoint_epoch=69-val_loss=0.01.ckpt')
    model = IKPoseTrainer.load_from_checkpoint(str(ckpt_path))
    model.eval()
    ...
    o_viz_path = f'/media/F/thesis/motion_capture/data/debug/{seq_kps_3d_path.stem}_breakdance.mp4'
