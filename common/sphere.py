...
class Sphere(object):
    ...
    def to_mesh(self, color=colors['red']):
        ...
        return trimesh.Trimesh(vertices=v * self.radius + self.center, faces=f,
    ...
    def intersection_vol(self, sphere):
        ...
        return (np.pi * (R + r - d) ** 2 * (d ** 2 + 2 * d * r - 3 * r * r + 2 * d * R + 6 * r * R - 3 * R * R)) / (
                12 * d)
...
def points_to_spheres(points, radius=0.01, vc=colors['red']):
    ...
    for id in range(len(points)):
        spheres.append(Sphere(center=points[id].reshape(-1, 3), radius=radius).to_mesh(
            color=vc if len(vc) == 3 and not isinstance(vc[0], list) else vc[id]))
    return spheres
